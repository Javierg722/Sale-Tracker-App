...
const TEMPLATE_WORKBOOK_PATH = "./Investment_Tracker_MASTER.xlsx";
const EXPORT_INPUT_COLUMNS = ["E","G","H","I","J","K","L","N"];
const intlTickers = new Set(["VXUS","IXUS","SCHF","FTIHX"]);
const DEFAULT_STATE = createEmptyState();
...
const MASTER_META_KEY = "sale-tracker-master-meta-v1";
const MASTER_DB_NAME = "sale-tracker-files-v1";
const MASTER_DB_STORE = "files";
const MASTER_DB_KEY = "currentMasterWorkbook";
const MERGE_API_URL = "https://sale-tracker-merge-api.vercel.app/api/merge";
...
function loadMasterWorkbookMeta(){
  try{
...
function saveMasterWorkbookMeta(meta){
  try{
    localStorage.setItem(MASTER_META_KEY, JSON.stringify(meta));
  } catch(err){
    console.warn("Could not save master workbook metadata", err);
  }
}
function openMasterDb(){
  return new Promise((resolve, reject) => {
    const request = indexedDB.open(MASTER_DB_NAME, 1);
    request.onupgradeneeded = () => {
      const db = request.result;
      if(!db.objectStoreNames.contains(MASTER_DB_STORE)){
        db.createObjectStore(MASTER_DB_STORE, { keyPath: "id" });
      }
    };
...
async function saveMasterWorkbookFile(file){
  const buffer = await file.arrayBuffer();
  const db = await openMasterDb();
  await new Promise((resolve, reject) => {
    const tx = db.transaction(MASTER_DB_STORE, "readwrite");
    const store = tx.objectStore(MASTER_DB_STORE);
    store.put({
      id: MASTER_DB_KEY,
      name: file.name,
  ...
  saveMasterWorkbookMeta({
    name: file.name,
    importedAt: new Date().toISOString(),
    size: file.size || buffer.byteLength
  });
}
function renderMasterWorkbookStatus(){
  const el = document.getElementById("masterWorkbookStatus");
  if(!el) return;
  const meta = loadMasterWorkbookMeta();
  if(!meta){
    el.innerHTML = '<span class="muted">No workbook imported yet. Importing a workbook makes it the current master template until you import another one.</span>';
    return;
  }
  const importedLabel = meta.importedAt ? new Date(meta.importedAt).toLocaleString() : "Unknown";
  const sizeLabel = typeof meta.size === "number" ? `${(meta.size / 1024 / 1024).toFixed(2)} MB` : "";
  el.innerHTML = `<strong>${meta.name || "Workbook"}</strong><br><span class="muted">Imported: ${importedLabel}${sizeLabel ? ` · ${sizeLabel}` : ""}</span>`;
}

async function getMasterWorkbookRecord(){
  const db = await openMasterDb();
  return await new Promise((resolve, reject) => {
    const tx = db.transaction(MASTER_DB_STORE, "readonly");
    const store = tx.objectStore(MASTER_DB_STORE);
    const req = store.get(MASTER_DB_KEY);
...
async function runSpreadsheetExport(){
  const master = await getMasterWorkbookRecord();
  if(!master || !master.bytes){
    throw new Error("No master workbook is saved yet. Import a workbook first.");
  }
  ...
  const payload = {
    ...
    rows: rowsForExport().map(row => ({
      ticker: row.ticker ?? "",
      buyDate: row.buyDate ?? "",
      sharesBought: row.sharesBought ?? null,
      costPerShare: row.costPerShare ?? null,
      sellDate: row.sellDate ?? "",
      sharesSold: row.sharesSold ?? null,
      salePricePerShare: row.salePricePerShare ?? null,
      note: row.note ?? ""
    }))
  };

  const workbookBlob = new Blob([master.bytes], {
    type: master.type || "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
  });
  const workbookFile = new File([workbookBlob], master.name || "Tracker MASTER.xlsx", {
    type: master.type || "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
  });

  const formData = new FormData();
  formData.append("workbook", workbookFile);
  formData.append("data", JSON.stringify(payload.rows));
  ...
  if(!response.ok){
    ...
    throw new Error(errorText || `Merge failed (${response.status})`);
  }

  const mergedBlob = await response.blob();
  const stamp = new Date().toISOString().slice(0,19).replace(/[:T]/g,"-");
  const baseName = (master.name || "Tracker MASTER.xlsx").replace(/\.xlsx$/i, "");
  const filename = `${baseName} - Merged ${stamp}.xlsx`;
...
function clearEntryRange(sheet, startRow, endRow){
  for(let row = startRow; row <= endRow; row += 1){
    for(const col of EXPORT_INPUT_COLUMNS){
...
function hasAdjustedBasis(row){
  return !row.washSale &&
    (row.basisAdjustmentIn || 0) > 0.000001 &&
    typeof row.adjustedCostPerShare === "number" &&
    typeof row.costPerShare === "number" &&
    Math.abs(row.adjustedCostPerShare - row.costPerShare) > 0.01;
}
...
function computeCalculatedModel(){
  ...
  basisIn[lot.id] !== undefined
    ...
    let adjustedSharesInLot = replacementMatchesForLot(lot.id).reduce((sum, match) => sum + Number(match.matchedShares || 0), 0);
    if(!(adjustedSharesInLot > 0) && basisAdjustmentIn > 0 && typeof adjustedCostPerShare === "number" && Math.abs(adjustedCostPerShare - lot.costPerShare) > 0.000001){
      const inferred = basisAdjustmentIn / Math.abs(adjustedCostPerShare - lot.costPerShare);
      if(Number.isFinite(inferred) && inferred > 0) adjustedSharesInLot = Math.min(lot.sharesBought || 0, inferred);
    }
    const unadjustedSharesInLot = Math.max(0, (lot.sharesBought || 0) - adjustedSharesInLot);
    const adjustedMatchedCostPerShare = adjustedSharesInLot > 0
      ? lot.costPerShare + (basisAdjustmentIn / adjustedSharesInLot)
      : null;
    const fullyAdjustedLot = adjustedSharesInLot > 0 && Math.abs(adjustedSharesInLot - (lot.sharesBought || 0)) <= 0.000001;
    ...
    return {
      ...
      adjustedSharesInLot,
      unadjustedSharesInLot,
      adjustedMatchedCostPerShare,
      fullyAdjustedLot,
      ...
      showAdjustedBasisTag: (!lotStats.washSale && basisAdjustmentIn > 0.000001 && typeof adjustedCostPerShare === "number" && Math.abs(adjustedCostPerShare - lot.costPerShare) > 0.01),
      matchStatus,
  ...
  document.getElementById("detailBody").innerHTML = `
    ...
    <div class="detail-section">
      ...
      <div class="detail-grid">
        ...
        ${r.basisAdjustmentIn > 0 && r.adjustedMatchedCostPerShare !== null ? (
          r.fullyAdjustedLot
            ? `<div class="detail-card"><span class="label">Adjusted Cost / Share</span><span class="value">${currency(r.adjustedMatchedCostPerShare)}</span></div>`
            : `<div class="detail-card"><span class="label">Original Cost / Share</span><span class="value">${currency(r.costPerShare)}</span></div>
               <div class="detail-card"><span class="label">Matched Shares @ Adj. Basis</span><span class="value">${num(r.adjustedSharesInLot)}</span></div>
               <div class="detail-card"><span class="label">Adj. Cost / Share (Matched)</span><span class="value">${currency(r.adjustedMatchedCostPerShare)}</span></div>
               <div class="detail-card"><span class="label">Unadjusted Shares @ Original Cost</span><span class="value">${num(r.unadjustedSharesInLot)}</span></div>`
...
document.getElementById("importXlsx").addEventListener("change", async (e) => {
  ...
  try{
    ...
    saveBaselineSnapshot();
    try{
      await saveMasterWorkbookFile(file);
    } catch(masterErr){
      console.warn("Could not save imported workbook as current master template", masterErr);
    }
    render();
    alert(`Spreadsheet import complete. Loaded ${state.lots.length} lots and ${state.sales.length} sales. This workbook is now your current master template until you import another one.`);
  } catch(err){
...
async function confirmExportReviewDialog(){
  try{
    ...
    alert("Workbook merge failed: " + err.message);
  }
}
